...
describe("POST /sign-up", () => {
  ...
  it("should respond with status 409 for an already signed e-mail", async () => {
    ...
    await createUser(SignUpbody.name, SignUpbody.email, SignUpbody.password);
...
describe("POST /sign-in", () => {
  ...
  it("should respond with status 200 for a valid login input", async () => {
    ...
    await createUser(SignInbody.name, SignInbody.email, SignInbody.password);
  ...
  it("should respond with status 401 when user exists but password is invalid", async () => {
    ...
    const response = await agent.post("/sign-in").send({ email: SignInbody.email, password: "invalid_password" });
  ...
  it("should respond with status 401 when user doesn't exist", async () => {
    ...
    const response = await agent.post("/sign-in").send({ email: "not_registered_email@email.com", password: SignInbody.password });
